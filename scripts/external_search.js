...
function init_search_list(sourceList) {
...
function show_external_search_box(searchQuestion) {
  ...
  $('.external-search-box #external-search-box-select2').autocomplete({
    ...
    source: function( request, response ) {
      var data = update_current_search_list(request.term);
      response(data);
    },
    ...
    select: function( event, ui ) {
      ...
      save_data(ui.item.Show); //save select item
      return false;
    },
...
function update_current_search_list(term){
  currentSearchList = [];
...
function hide_external_search_box() {
  //$('.rt-body.slt-page-container main').find('section').show();
  $('.external-search-box').hide();
}
